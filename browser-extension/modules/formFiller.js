...
const formFiller = (() => {
  ...
  // New function to enable click-to-fill functionality
  async function enableClickToFill(fieldValues) {
    console.log("Enabling click-to-fill functionality with", fieldValues.length, "fields");
    
    // Store field values in a map for quick lookup
    const fieldMap = new Map();
    
    // Process all field values to build lookup mappings
    fieldValues.forEach(field => {
      // Map by ID if available
      if (field.id) {
        fieldMap.set(field.id.toLowerCase(), field);
      }
      ...
      if (field.label) {
        fieldMap.set(field.label.toLowerCase(), field);
      }
      
      // Map by aria-label if available
      if (field.ariaLabel) {
        fieldMap.set(field.ariaLabel.toLowerCase(), field);
      }
    });
    
    // Helper function to find matching field value
    function findMatchingField(element) {
      // Try to match by id
      if (element.id && fieldMap.has(element.id.toLowerCase())) {
        return fieldMap.get(element.id.toLowerCase());
      }
      
      // Try to match by name
      if (element.name && fieldMap.has(element.name.toLowerCase())) {
        return fieldMap.get(element.name.toLowerCase());
      }
      
      // Try to match by aria-label
      const ariaLabel = element.getAttribute('aria-label');
      if (ariaLabel && fieldMap.has(ariaLabel.toLowerCase())) {
        return fieldMap.get(ariaLabel.toLowerCase());
      }
      
      // Try to find by label relationship
      if (element.id) {
        const label = document.querySelector(`label[for="${element.id}"]`);
        if (label && label.textContent) {
          const labelText = label.textContent.trim().toLowerCase();
          // Check for exact match
          if (fieldMap.has(labelText)) {
            return fieldMap.get(labelText);
          }
    ...
    // Create an overlay indicator to show which fields can be filled
    function createOverlayIndicator() {
      const indicator = document.createElement('div');
      indicator.id = 'form-master-click-indicator';
    ...
    const indicator = createOverlayIndicator();
    
    // Add visual hover effect to indicate fillable fields
    function addHoverEffect(event) {
      const element = event.target;
      
      // Only show for input elements, selects, and textareas
      ...
      const field = findMatchingField(element);
      if (!field) {
        indicator.style.display = 'none';
        return;
      }
      
      // Show indicator for fillable field
      const rect = element.getBoundingClientRect();
    ...
    async function handleFormFieldClick(event) {
      const element = event.target;
      ...
      if (!field || field.value === undefined || field.value === null) {
        console.log("No matching field value found for:", element);
        return;
      }
      
      // Get field info needed for filling
      const tagName = element.tagName.toLowerCase();
      const inputType = element.type ? element.type.toLowerCase() : '';
      const value = field.value;
      
      console.log(`Filling ${tagName} field with value: ${value}`);
      
      try {
        // Use existing fill field function
        await fillField(element, tagName, inputType, value);
        console.log(`Successfully filled field via click: ${field.label || field.name || field.id}`);
      } catch (error) {
        console.error(`Error filling field on click:`, error);
      }
    }
    
    // Add event listeners
    document.addEventListener('mouseover', addHoverEffect, true);
    document.addEventListener('click', handleFormFieldClick, true);
    
    // Return a function to disable click-to-fill
    return function disableClickToFill() {
      document.removeEventListener('mouseover', addHoverEffect, true);
      document.removeEventListener('click', handleFormFieldClick, true);
  ...
  return {
    performFormFilling,
    enableClickToFill // Export the new function
  };
})();
